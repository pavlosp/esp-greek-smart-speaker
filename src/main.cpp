...
static void initialize_sntp(void)
{
    ...
    // const char server[] = "time.google.com";
...
static void obtain_time(void)
{
    ...
    while (timeinfo.tm_year < (2016 - 1900)) {
        ...
        ESP_LOGI(TAG, "timeinfo.tm_year is %d; tm_mon is %d; tm_mday is %d", timeinfo.tm_year, timeinfo.tm_mon, timeinfo.tm_mday);
    }
    ...
    void *user_data)
{
    ...
    if (params != NULL && params->message.topic != NULL) {
        ...
        if (strncmp(subscribe_topic_command, params->message.topic, strlen(subscribe_topic_command)-2) == 0) {
            int value;
            sscanf(sub_message, "{\"outlet\": %d}", &value);
            ESP_LOGI(TAG, "value: %d", value);
            if (value == 1) {
                gpio_set_level((gpio_num_t) OUTPUT_GPIO, true);
            } else if (value == 0) {
                gpio_set_level((gpio_num_t) OUTPUT_GPIO, false);
            }
        }
        free(sub_message);
    }
...
void app_main()
{
  ...
  //I2SOutput *i2s_output = NULL; // new I2SOutput();
  //i2s_output->start(I2S_NUM_1, i2s_speaker_pins);
  //Speaker *speaker = NULL; //new Speaker(i2s_output);
  ...
  Application *application = new Application(i2s_sampler, NULL, NULL, NULL);
